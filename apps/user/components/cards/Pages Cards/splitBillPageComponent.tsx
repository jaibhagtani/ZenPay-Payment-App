...
interface SplitBillPageComponent {
...
export default function SplitBillPageComponent({paymentsPending, pendingCredits, totalSplits, activeSplits, allSplits} : SplitBillPageComponent) {
...
function Card({ icon, label, value, classes }: { icon: React.ReactNode; label: string; value: string; classes: string }) {
  return (
    <div className={`p-4 rounded-xl shadow-sm ${classes} border`}>
      ...
      <p className={`text-2xl font-semibold ${classes.split(" ").pop()}`}>{value.toString()}</p>
    </div>
  );
}
